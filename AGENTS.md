...
- Own the viewer-facing UFC schedule seed in `src/config/ufcSchedule.ts`; refresh it with `npm run update:schedule`.
...
`src/config/ufcSchedule.ts` is generated by `scripts/update-ufc-schedule.mjs`.
Run `npm run update:schedule` before changing schedule UI or deploying near a
fight weekend. The updater reads the public Paramount+ UFC schedule page,
converts ET/PT start times to UTC, keeps a fallback seed for resilience, and
uses `dateIso` only for sorting cards whose stream time is still TBA.

`.github/workflows/update-ufc-schedule.yml` runs twice weekly and opens a PR
when the generated seed changes. Keep generated IDs unique because React
schedule rows and tests depend on stable event IDs.
